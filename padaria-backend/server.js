...
async function deletarPedidoPorID(id) {
  const pool = new Pool(credentials);
  const now = await pool.query(`DELETE from pedidos where id='${id}'`);
  await pool.end();
  ...
  function (req, res) {
    ...
    if (queroentrega == true) {
      CadastrarPedido(endereco.nome, pedidoCompleto, total.toFixed(2)).then(
        () => {
          res.redirect(
            "https://api.whatsapp.com/send?1=pt_BR&phone=558893185656&text=" +
              pedidoCompleto
          );
        }
      );
...
app.get("/api/deletarPedido/:id", function (req, res) {
  var id = req.params.id;
  deletarPedidoPorID(id).then(() => {
    res.json("ok");
  });
});
